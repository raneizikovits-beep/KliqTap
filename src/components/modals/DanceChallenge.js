// DanceChallenge.js — Production v4.0
// A dance-challenge recorder: learn a trending dance from the reference video on
// top, record yourself dancing below, get a live "move match" score, add FX, and
// publish to Feed or Pulse.
// 🆕 Audio FX (Echo / Reverb / Hall / Harmony / Pitch) baked on the take
// 🆕 Co-Dancer: invite a friend to a real-time WebRTC dance duet
// 🆕 Publish to Feed (→ Profile) or Pulse, with a caption
// 🆕 Web (Chrome) support: YouTube playlist iframe fallback
// 🐛 mode="video" recording, stable source, square bottom corners,
...
const DANCE_SOURCE = { uri: 'https://m.youtube.com/results?search_query=trending+tiktok+dance+tutorial' };
...
const DANCE_PLAYLIST_ID = 'PLirAqAtl_h2r5g8xGajEwdXd3x1sZh8hC';
const DANCE_WEB_SRC = `https://www.youtube.com/embed/videoseries?list=${DANCE_PLAYLIST_ID}&autoplay=1&rel=0`;
...
const SYNC_EVENTS = [
  { text: 'ON BEAT 🎯',      color: '#FF006E' },
  { text: 'IN THE ZONE 🔥',  color: '#FF9F1C' },
  { text: 'CLEAN MOVES 💜',  color: '#B14EFF' },
  { text: 'MIRROR MODE ✨',   color: '#00F5D4' },
  { text: 'RHYTHM MATCH 🎵', color: '#3A86FF' },
];

const getRating = (peak) => {
  if (peak >= 85) return { text: 'DANCE GOD 🏆',  color: '#FFD700' };
  if (peak >= 65) return { text: 'GREAT MOVES 🔥', color: '#00F5D4' };
  return               { text: 'KEEP DANCING! 💪', color: '#FF006E' };
};
...
const DanceStage = React.memo(function DanceStage({ height }) {
  return (
    <View style={[styles.sourceSegment, { height }]}>
      ...
      <View style={styles.segLabel}><Text style={styles.segLabelText}>TUTORIAL</Text></View>
    </View>
  );
});
...
// ─── Component ────────────────────────────────────────────────────────────────
export const DanceChallenge = ({ sheet, onClose, isDark, supabase, currentUser }) => {
  ...
  const trend     = sheet?.trend || '#DanceChallenge';
  ...
  const startRecording = async () => {
    ...
    } catch (e) { console.error('[DanceChallenge] record error:', e); }
  ...
  const publishDuet = async () => {
    ...
    const text     = caption?.trim() || `💃 Dance challenge — ${syncPeak}% match`;

    setPosting(true);
    try {
      const form = new FormData();
      form.append('image', { uri: recordedDuet, name: 'dance.mp4', type: 'video/mp4' });
      ...
      Alert.alert(
        'Posted! 🎉',
        isPulse ? 'Your dance is live as a Pulse for 24 hours.' : 'Your dance is live on your Feed and Profile.',
      );
      onClose?.();
    } catch (e) {
      console.error('[DanceChallenge] publish error:', e?.message || e);
      Alert.alert('Upload failed', 'Could not post your dance. Please try again.');
    } finally {
      setPosting(false);
    }
  };
  ...
  if (hasPermission === null || hasAudio === null) return (
    <View style={styles.loadingScreen}>
      ...
      <Text style={styles.loadingText}>Setting up dance studio...</Text>
    </View>
  );
      ...
      <View style={styles.container}>
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <TextInput
            ...
            placeholder="Say something about your dance…"
          ...
          <View style={styles.previewFooter}>
            ...
            >
              {posting ? (
                ...
                <>
                  <Text style={[styles.postBtnText, { color: rating.color === '#FFD700' ? '#000' : '#fff' }]}>Post Dance</Text>
    ...
    <View style={styles.container}>
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <Animated.View style={[styles.splitWrap, { transform: [{ scale: layoutAnim }] }]}>
          ...
          <DanceStage height={layout.sourceH} />

          {/* Divider */}
          <View style={styles.splitDivider}>
            <View style={styles.divLine} />
            <View style={styles.divPill}><Text style={styles.divText}>DANCE</Text></View>
            <View style={styles.divLine} />
          </View>
          ...
          <View style={styles.cameraSegment}>
            ...
            {duetSetup && (
              <View style={styles.lobby}>
                ...
                <Text style={styles.lobbyTitle}>Dance with a friend, live</Text>
        ...
        {duetLive && (
          <View style={styles.footer}>
            <TouchableOpacity style={styles.leaveBtn} onPress={exitDuet} activeOpacity={0.85}>
              ...
              <Text style={styles.btnText}>End Dance</Text>
            </TouchableOpacity>
          </View>
        )}
